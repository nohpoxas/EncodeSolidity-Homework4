...
export class AppService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.infuraKey = this.configService.get<string>('INFURA_API_KEY');
    ...
    this.tokenAddress = this.configService.get<string>(
      'TOKEN_CONTRACT_ADDRESS',
    );
    this.tokenContract = new MyToken__factory(this.wallet).attach(
      this.tokenAddress,
    );
  }
  ...
  async claimTokens(address: string) {
    ...
    try {
      ...
      return {
        ...
        error: e.reason,
        myMessage:
          'probably the signer on the server has not the minter role, check on goerli scan',
  ...
  importInfuraApiKey() {
    return { result: this.infuraKey };
    // return this.infuraKey;
  }

  importEtherscanApiKey() {
    return { result: this.etherscanKey };
    // return this.etherscanKey;
  }

  importWallet() {
    return { result: this.privateKey };
